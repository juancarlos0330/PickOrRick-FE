...
import style from "./PresaleSwapModal.module.scss";
...
const pickorricktesttokenaddress = "0x75430D0782A443bD4f1c92C69009599dEA53A206";
const presaletokenaddress = "0xD6fF052Bb01559A159ac44A1f54af21A4cc02AF6";

const EthToRickPrice = 680900000000000;

const PresaleSwapModal: FC<IPresaleSwapModal> = (props: any) => {
  ...
  const [wethtokenAmount, setWethTokenAmount] = useState<any>(0);
      ...
      if ((window as any).ethereum) {
        (window as any).ethereum
          ...
          .then(async (accounts: string[]) => {
            ...
            try {
              const gettokenprice = await PresaleTokenContracts.tokenPrice();
              setTokenPrice(
                Number(Number(String(gettokenprice)) / 1000000000000000000)
              );
            } catch (err) {
              setTokenPrice(0);
              console.log(err);
  ...
  const onSubmit = async (
    ...
    try {
      if (PresaleTokenContract) {
        try {
          setSwapFlag(true);
          const presaleresult = await PresaleTokenContract.buyTokens({
            from: account,
            value: ethers.utils.parseUnits(String(wethtokenAmount), 18),
            gasLimit: 200000,
          });
          await presaleresult.wait();
          setSwapFlag(false);
        } catch (err) {
          console.log(err);
          setSwapFlag(false);
  ...
  const getAssets = async () => {
    if (PresaleTokenContract) {
      try {
        setSwapFlag(true);
        // const gettokenprice = await PresaleTokenContract.tokenPrice();
        // setTokenPrice(
        //   Number(Number(String(gettokenprice)) / 1000000000000000000)
        // );
        ...
        // );
        setRickTokenAmount(
          Number(Number(wethtokenAmount) * Number(EthToRickPrice))
        );
        setSwapFlag(false);
      } catch (err) {
        console.log(err);
      ...
      <div className={style.chooseModal}>
        ...
        <form onSubmit={formik.handleSubmit} className={style.form}>
          <TextField
            ...
            onChange={(e) => setWethTokenAmount(e.target.value)}
...
export default connect(mapStateToProps, { saveWalletStatus })(PresaleSwapModal);
